...
# Pyrogram ke purane versions mein channel-ID boundary outdated ho sakti hai.
# Hamare STORAGE_CHANNEL jaise naye -100xxxxxxxxxx IDs ko valid banane ke liye
# runtime par correct lower boundary set kar rahe hain.
# Reference: Pyrogram PR #1430.
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        print(f"✅ Telegram client connected: {bot.is_connected}")
        print(f"✅ Telegram client initialized: {bot.is_initialized}")
        print("📡 Telegram update receiver is active; waiting for messages...")
        ...
        try:
            storage_chat = await bot.get_chat(Config.STORAGE_CHANNEL)
            print(
                f"✅ Storage channel accessible hai: "
                f"title={getattr(storage_chat, 'title', None)!r}, id={getattr(storage_chat, 'id', None)}"
            )
        except Exception as storage_error:
            print(f"!!! STORAGE CHANNEL ERROR !!!")
            print(f"Configured STORAGE_CHANNEL: {Config.STORAGE_CHANNEL}")
            print(f"Telegram/Pyrogram error: {storage_error}")
            print("Bot ko stop nahi kiya ja raha; Telegram updates continue rahenge.")
    ...
    try:
        ...
        print(f"Shutdown warning: {stop_error}")
...
async def start_command(client: Client, message: Message):
    ...
    print(f"📩 /start received | user_id={user_id} | username={message.from_user.username}")
...
@bot.on_message(filters.private & ~filters.command("start") & ~(filters.document | filters.video | filters.audio))
async def diagnostic_private_message(client: Client, message: Message):
    print(
        f"📨 Private message received | user_id={message.from_user.id} | "
        f"text={message.text!r}"
    )
    await message.reply_text(
        "✅ Bot online hai. File/document/video/audio bhejiye."
    )


async def handle_file_upload(message: Message, user_id: int):
    print(
        f"📁 File received | user_id={user_id} | "
        f"document={bool(message.document)} video={bool(message.video)} audio={bool(message.audio)}"
    )
    try:
