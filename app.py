...
async def private_message_probe(client: Client, message: Message):
    """Low-level diagnostic: proves Telegram private messages reach Pyrogram."""
    try:
        text = message.text or message.caption or "<non-text>"
        print(
            f"📡 Telegram update received | chat_id={message.chat.id} | "
            f"user_id={message.from_user.id if message.from_user else 'unknown'} | "
            f"content={text[:120]!r}"
        )
    except Exception:
        print("📡 Telegram update received (diagnostic log failed to format message)")
...
async def validate_and_start_bot():
    ...
    print("🧪 Diagnostic private-message handler is registered; send /start from Telegram.")
