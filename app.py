...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        try:
            # Pyrogram accepts both integer IDs and @usernames. Keep the configured
            # value unchanged, but provide a clear diagnostic if Telegram rejects it.
            storage_chat = await bot.get_chat(Config.STORAGE_CHANNEL)
            print(f"✅ Storage channel accessible hai: {storage_chat.title}")
        except Exception as e:
            print("!!! STORAGE CHANNEL ERROR !!!")
            print(f"Configured STORAGE_CHANNEL: {Config.STORAGE_CHANNEL!r}")
            print("Check that this is the real Telegram channel ID (normally -100xxxxxxxxxx),")
            print("and that the bot is an administrator in that channel.")
            print(f"Telegram/Pyrogram error: {e}")
            # Do not hide the real problem inside a generic startup traceback.
            # Keep the web server alive so Render remains healthy.
            storage_channel_error = True
